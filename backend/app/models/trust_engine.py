...
class LivenessLog(Base):
    """
    Stores every passive + active liveness check attempt.
    Covers both /api/liveness/check and /api/liveness/challenge.
    """
    ...
    # Passive liveness
    liveness_score: Mapped[float | None] = mapped_column(Float, nullable=True, comment="0.0–1.0")
    is_live: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="false")
    confidence: Mapped[float | None] = mapped_column(Float, nullable=True, comment="0–100")
    ...
    challenge_completed: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="false")
    ...
    def __repr__(self) -> str:
        return f"<LivenessLog id={self.id} is_live={self.is_live} type={self.check_type}>"
...
class EmotionLog(Base):
    """Stores emotion analysis results (secondary liveness signal)."""
    ...
    confidence: Mapped[float] = mapped_column(Float, nullable=False, comment="0–100")
    all_scores: Mapped[dict | None] = mapped_column(JSONB, nullable=True, comment="Per-class probabilities")
    ...
    def __repr__(self) -> str:
        return f"<EmotionLog id={self.id} emotion={self.emotion} conf={self.confidence}>"
...
class HeadPoseLog(Base):
    """Stores head pose estimation results (pitch, roll, yaw)."""

    __tablename__ = "headpose_logs"
    ...
    pitch: Mapped[float | None] = mapped_column(Float, nullable=True, comment="Degrees, positive=up")
    roll: Mapped[float | None] = mapped_column(Float, nullable=True, comment="Degrees, positive=clockwise")
    yaw: Mapped[float | None] = mapped_column(Float, nullable=True, comment="Degrees, positive=right")

    is_frontal: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default="true")
    session_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ip_address: Mapped[str | None] = mapped_column(String(45), nullable=True)
    ...
    def __repr__(self) -> str:
        return f"<HeadPoseLog id={self.id} pitch={self.pitch} yaw={self.yaw} roll={self.roll}>"
...
class DeepfakeLog(Base):
    """Records all deepfake detection analysis attempts."""

    __tablename__ = "deepfake_logs"
    ...
    deepfake_probability: Mapped[float] = mapped_column(Float, nullable=False, comment="0.0–1.0")
    is_deepfake: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="false")
    ...
    image_hash: Mapped[str | None] = mapped_column(String(64), nullable=True, comment="SHA-256 of analyzed frame (not stored)")
    ...
    def __repr__(self) -> str:
        return f"<DeepfakeLog id={self.id} is_deepfake={self.is_deepfake} prob={self.deepfake_probability}>"
